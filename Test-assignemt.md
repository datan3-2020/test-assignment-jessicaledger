...
``` r
table(Data$i_sex_dv, Data$i_ypsocweb, Data$i_age_dv)
```
...
Data$account <- Data$i_ypsocweb
Data$account[Data$account == -9] <- NA
Data$account[Data$account == 2] <- 0
...
tapply(Data$account, Data$i_age_dv, mean, na.rm = TRUE)
```
...
``` r
tapply(Data$account, Data$gender, mean, na.rm = TRUE)
```

    ##         0    female      male 
    ## 1.0000000 0.8395150 0.7826087

## Write short interpretation (10 points)

The probability of having a social media account increases with age.
From 0 at 9 years old to 1 at 16 years old. The biggest increase in
likelihood of having social media is seen between the ages of 10-11
(0.4868996 0.6989899)

The probability of having a social media account is more common with
females over males. But both are highly likely of having an account (84%
for females and 78% for males)
...
``` r
ggplot(Data,aes(x=i_age_dv, y=account)) + 
    stat_summary(fun.y=mean, geom="bar") + facet_grid(~gender)
```
